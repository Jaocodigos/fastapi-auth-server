...
from app.services.security.auth import token_or_admin_authentication
...
def list_claims(db: Session = Depends(get_db),
                _: None = Depends(token_or_admin_authentication)):

    return get_claims(db)

@router.post("/claims", status_code=201)
def register_claims(payload: Annotated[ClaimCreate, Body()],
